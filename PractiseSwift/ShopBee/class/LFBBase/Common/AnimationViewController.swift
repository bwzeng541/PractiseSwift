...
class AnimationViewController: BaseViewController, CAAnimationDelegate {
    ...
    func addProductsAnimation(imageView: UIImageView) {
        ...
        let p3 = CGPoint.init(x: view.width - view.width / 4 - view.width / 8 - 6, y: self.view.layer.bounds.size.height - 40)
        
        let positionAnimation = CAKeyframeAnimation(keyPath: "position")
        let path = CGMutablePath();
        path.move(to: CGPoint.init(x: p1.x, y: p1.y))
        path.addCurve(to: p3, control1: CGPoint.init(x: p1.x, y: p1.y-30), control2:  CGPoint.init(x: p3.x, y: p1.y-30))
    ...
    func addProductsToBigShopCarAnimation(_ imageView: UIImageView) {
        ...
        let p1 = transitionLayer.position;
        let p3 = CGPoint.init(x: view.width - 40, y: self.view.layer.bounds.size.height - 40)
