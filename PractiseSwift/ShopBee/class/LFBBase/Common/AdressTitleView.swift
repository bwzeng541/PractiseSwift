...
class AdressTitleView: UIView {
    ...
    private let arrowImageView = UIImageView(image: R.image.allowBlack())
    ...
    override init(frame: CGRect) {
        ...
        playLabel.frame = CGRect.init(x: 0, y: (frame.size.height - playLabel.height - 2) * 0.5, width: playLabel.frame.size.width + 6, height: playLabel.frame.size.height + 2)
        ...
        titleLabel.frame = CGRect.init(x: playLabel.frame.maxX + 4, y: 0, width: titleLabel.width, height: frame.height)
        addSubview(titleLabel)
        
        arrowImageView.frame = CGRect.init(x: titleLabel.frame.maxX + 4, y: (frame.size.height - 6) * 0.5, width: 10, height: 6)
    ...
    func setTitle(_ text: String) {
        ...
        titleLabel.sizeToFit()
        titleLabel.frame = CGRect.init(x: playLabel.frame.maxX + 4, y: 0, width: titleLabel.width, height: frame.height)
        arrowImageView.frame = CGRect.init(x: (titleLabel.frame.maxX) + 4, y: (frame.size.height - arrowImageView.height) * 0.5, width: arrowImageView.width, height: arrowImageView.height)
        adressWidth = arrowImageView.frame.maxX
    }
}
